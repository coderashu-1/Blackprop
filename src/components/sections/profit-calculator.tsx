...
function LockIcon() {
  return (
    <svg viewBox="0 0 20 20" fill="none" className="h-4 w-4" aria-hidden="true">
      <rect x="4" y="8" width="12" height="9" rx="2.3" stroke="currentColor" strokeWidth="1.4" />
      <path d="M6.5 8V6.2A3.5 3.5 0 0 1 10 2.7a3.5 3.5 0 0 1 3.5 3.5V8" stroke="currentColor" strokeWidth="1.4" strokeLinecap="round" />
    </svg>
  );
}
...
function ArrowRight() {
  return (
    <svg viewBox="0 0 20 20" fill="none" className="h-4 w-4 transition-transform duration-300 group-hover:translate-x-1" aria-hidden="true">
      <path d="M3.5 10h12M11.5 6l4 4-4 4" stroke="currentColor" strokeWidth="1.7" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function DragIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-3.5 w-3.5" aria-hidden="true">
      <path d="M4 12h16M7 9l-3 3 3 3M17 9l3 3-3 3" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round" />
    </svg>
  );
}
...
function AccountIcon() {
  return (
    <svg viewBox="0 0 24 24" fill="none" className="h-[17px] w-[17px]" aria-hidden="true">
      ...
      <path d="M7 10h10M7 14h5" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" />
    </svg>
  );
}
  ...
  return (
    <div className="relative mx-auto mt-10 h-[260px] w-full max-w-[480px] sm:h-[300px] lg:mx-0 lg:mt-12 lg:h-[330px]">
      ...
      <div className="bp-logo-card absolute right-[5%] top-[12%] grid h-[165px] w-[165px] place-items-center rounded-[38px] border border-white/10 bg-[#0B0B0B] shadow-[0_28px_65px_rgba(0,0,0,.28)] sm:h-[190px] sm:w-[190px] sm:rounded-[44px]">
        ...
        <BPMark width={62} height={78} color="#FFFFFF" className="drop-shadow-[0_5px_8px_rgba(0,0,0,.5)] sm:w-[70px]" />
      </div>
    </div>
  );
}
...
export function ProfitCalculator() {
  ...
  const profit = useMemo(() => {
    return accountSize * (profitRate / 100) * (profitSplit / 100);
  ...
  return (
    <section id="profit-calculator" className="relative overflow-hidden bg-[#FBFAF6] py-14 text-black sm:py-16 lg:py-20 xl:py-24">
      <style>{`
        @keyframes bpFadeUp {
        ...
        @keyframes bpThumbPulse {
          0%, 100% { box-shadow: 0 3px 10px rgba(0,0,0,.16), 0 0 0 0 rgba(212,175,55,.20); }
          50% { box-shadow: 0 3px 10px rgba(0,0,0,.16), 0 0 0 8px rgba(212,175,55,.10); }
        }
        @keyframes bpResultPop {
          0% { opacity: .75; transform: translateY(4px); }
          100% { opacity: 1; transform: translateY(0); }
        }
        .bp-section-copy { animation: bpFadeUp .7s cubic-bezier(.2,.75,.25,1) both; }
        .bp-calculator-card { animation: bpFadeUp .75s .12s cubic-bezier(.2,.75,.25,1) both; }
        .bp-logo-card { animation: bpLogoFloat 4.6s ease-in-out infinite; }
        .bp-gold-card { animation: bpCardFloat 5.3s ease-in-out infinite; }
        .bp-back-card { animation: bpBackFloat 5.8s ease-in-out infinite; }
        .bp-glow { animation: bpGlowPulse 4.8s ease-in-out infinite; }
        .bp-drag-hint { animation: bpDragHint 1.35s ease-in-out infinite; }
        .bp-thumb-idle { animation: bpThumbPulse 1.8s ease-in-out infinite; }
        .bp-result-pop { animation: bpResultPop .22s ease-out both; }
        @media (prefers-reduced-motion: reduce) {
          .bp-section-copy, .bp-calculator-card, .bp-logo-card, .bp-gold-card,
          .bp-back-card, .bp-glow, .bp-drag-hint, .bp-thumb-idle, .bp-result-pop {
      ...
      <div className="mx-auto max-w-[1320px] px-4 sm:px-6 lg:px-8">
        <div className="grid items-center gap-10 lg:grid-cols-[1fr_.92fr] lg:gap-14 xl:gap-20">
          ...
          <div className="bp-calculator-card rounded-[28px] border border-black/[0.07] bg-white p-5 shadow-[0_30px_90px_rgba(54,43,16,.08)] sm:p-7 lg:p-8 xl:p-9">
            ...
            {/* ACCOUNT SIZE */}
            <div className="mt-7">
              <label id="profit-account-size-label" className="text-[11px] font-semibold text-black/45 sm:text-[12px] lg:text-[13px]">
                Account size
              </label>

              <div ref={accountDropdownRef} className="relative z-40 mt-2.5">
                <button
                  type="button"
                  ...
                  onClick={() => setAccountDropdownOpen((current) => !current)}
                  className={`group relative flex h-[58px] w-full items-center justify-between overflow-hidden rounded-2xl border px-4 text-left outline-none transition-all duration-300 sm:h-[64px] sm:px-5 lg:h-[68px] ${
                    accountDropdownOpen
                ...
                >
                  <div
                    className={`pointer-events-none absolute -right-10 top-1/2 h-24 w-24 -translate-y-1/2 rounded-full bg-[#D4AF37]/10 blur-2xl transition-opacity duration-300 ${
                      accountDropdownOpen ? "opacity-100" : "opacity-0"
                    }`}
                  />

                  <div className="relative flex min-w-0 items-center gap-3">
                    <div
                      className={`grid h-9 w-9 shrink-0 place-items-center rounded-xl transition-all duration-300 sm:h-10 sm:w-10 ${
                        accountDropdownOpen
                  ...
                  <div
                    className={`relative grid h-9 w-9 shrink-0 place-items-center rounded-xl text-black/40 transition-all duration-300 group-hover:bg-black/[0.04] group-hover:text-black/70 ${
                      accountDropdownOpen ? "rotate-180 bg-black/[0.04] text-black" : ""
                    }`}
                  >
                    <ChevronDown />
                  </div>
                ...
                <div
                  className={`absolute left-0 right-0 top-full z-50 mt-2 origin-top overflow-hidden rounded-[20px] border border-black/[0.07] bg-white p-2 shadow-[0_24px_70px_rgba(30,25,10,.16)] transition-all duration-200 ${
                    accountDropdownOpen
                ...
                >
                  ...
                  <div role="listbox" aria-labelledby="profit-account-size-label" className="max-h-[300px] overflow-y-auto overscroll-contain pr-1">
                    {accountSizes.map((size) => {
                      ...
                      return (
                        <button
                          ...
                          className={`group/option relative mb-1 flex min-h-[48px] w-full items-center justify-between overflow-hidden rounded-[13px] px-3.5 text-left transition-all duration-200 last:mb-0 ${
                            selected ? "bg-[#0B0B0B] text-white" : "text-black hover:bg-[#F6F5F1]"
                          }`}
                        >
            ...
            <div className="mt-7">
              <div className="flex items-end justify-between">
                <label htmlFor="profit-rate" className="text-[11px] font-semibold text-black/45 sm:text-[12px] lg:text-[13px]">
                  Profit rate
                </label>
              ...
              <div
                className={`mt-3 flex min-h-[28px] items-center transition-all duration-300 ${
                  hasDragged ? "pointer-events-none -translate-y-1 opacity-0" : "opacity-100"
                }`}
              >
              ...
              <div className="relative mt-2 h-10">
                <div className="absolute inset-x-0 top-1/2 h-[6px] -translate-y-1/2 rounded-full bg-black/[0.06]">
                  <div
                    ...
                    style={{ width: `${Math.max(0, Math.min(100, sliderPosition))}%` }}
                  />
                </div>
                ...
                <input
                  ...
                  onChange={(event) => updateProfitRate(Number(event.target.value))}
                ...
                <div
                  className={`pointer-events-none absolute top-[-25px] z-10 -translate-x-1/2 transition-all duration-150 ${
                    isDragging ? "translate-y-0 opacity-100" : "translate-y-1 opacity-0"
                  }`}
                  style={{ left: `${Math.max(0, Math.min(100, sliderPosition))}%` }}
                >
            ...
            <div className="mt-8 rounded-[22px] bg-[#0B0B0B] p-5 text-white sm:p-6">
              ...
              <div key={`${accountSize}-${profitRate}`} className="bp-result-pop mt-3 flex items-end gap-2.5" aria-live="polite">
                <span className="text-[2.45rem] font-black leading-none tracking-[-0.055em] sm:text-[3rem] lg:text-[3.35rem]">
                  {formatMoney(profit)}
                </span>
                <span className="mb-1 text-[10px] text-white/35 sm:text-[11px]">/ month</span>
              </div>

              <div className="mt-4 flex items-center justify-between border-t border-white/[0.08] pt-4">
                <span className="text-[10px] font-medium text-white/40 sm:text-[11px]">Profit split</span>
                <span className="text-[13px] font-black text-[#E5C65D] sm:text-[14px]">{profitSplit}%</span>
              </div>
            </div>

            {/* CTA */}
            <a href="#challenges" className="group mt-5 flex min-h-[54px] w-full items-center justify-center gap-2 rounded-2xl bg-[#D4AF37] px-5 text-[14px] font-black text-black transition duration-300 hover:-translate-y-0.5 hover:bg-[#E3C45C] hover:shadow-[0_12px_28px_rgba(181,135,30,.15)] sm:min-h-[58px] sm:text-[15px] lg:text-base">
              Start Earning
              <ArrowRight />
            </a>
